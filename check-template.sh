#!/usr/bin/env bash
# `git remote get-url` reads .git/config and does not contact the remote. The template is
# the one repository whose origin is TEMPLATE_ORIGIN below, and anything else — a
# different origin, or no origin at all — is a copy
set -euo pipefail
...
With no flag it works out which it is looking at from `git remote get-url origin`. So a
fork needs no flag, no edited workflow and no remembering, which is the point: forgetting
is the failure mode this guards, and a guard you have to arm by hand does not guard that
